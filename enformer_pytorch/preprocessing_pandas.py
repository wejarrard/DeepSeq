...
def check_overlap(row, df, overlap) -> list:
...
def get_cell_line_labels(cell_lines_directory) -> list:
...
def consolidate_csvs(cell_lines_directory, overlap_fraction=0.1) -> pd.DataFrame:
    assert os.path.exists(
        cell_lines_directory
    ), f"{cell_lines_directory} does not exist."
    assert os.path.isdir(
